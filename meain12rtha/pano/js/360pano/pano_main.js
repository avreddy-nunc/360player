...
var renderMarker = function() {
    ...
    loadData(function (data) {
        ...
        /*var sampleViewer = new PhotoSphereViewer({
            container : 'hidden-viewer',
            panorama : data.pano_image,
            time_anim: false,
        });
        sampleViewer.on("ready", function () {
            var pano_canvas = $('#hidden-viewer .psv-canvas-container canvas')[0];
            var degrees = [0,45,90,135,180,225,270,315];
            var features = document.getElementById('feature-thumbs').innerHTML;
            degrees.forEach(function (degree) {
                sampleViewer.rotate({longitude:(degree * Math.PI/180),latitude:0}, function () {
                    features += "<div data-fid='int-"+degree+"' class='slide-item'>" +
                        "<img src='" + pano_canvas.toDataURL() + "' alt='feature' />" +
                        "<div class='slide-title'>Interior " + degree + "<sup>o</sup></div></div>"
                });
        ...
        if(_p.isThumbsPlayer){
            ...
            degrees.forEach(function (data) {
                features += "<div data-fid='int-"+data.degree+"' class='slide-item'>" +
                    ...
                    "<div class='slide-title'>Interior " + data.degree + "<sup>o</sup></div></div>"
            });
            document.getElementById('feature-thumbs').innerHTML = features;
...
function generateHtmlTemplate(id,title, image, description){
    ...
    if(image){
        ...
        imageElem.innerHTML = '<div class="highlight-image-close-button">\n' +
            '                <img class="fa-times info-close-icon" style="display: block;width: 20px;height: 20px;" src="./img/times-solid.svg" alt="...">\n' +
            '            </div>\n' +
            '            <img src="'+image+'" />';
